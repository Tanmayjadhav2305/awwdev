...
export default function TechPartners() {
    return (
        <section className="py-1 border-y border-white/10 bg-[#030303] overflow-hidden relative select-none">
            {/* Ambient Center Glow to brighten the dark section */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-16 bg-primary-500/10 blur-[60px] pointer-events-none rounded-full" />
                ...
                >
                    {services.map((service, i) => (
                        <div key={i} className="flex items-center justify-center gap-8 md:gap-12 cursor-default relative z-20">
                            <span className="text-lg md:text-xl font-black whitespace-nowrap text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-500 uppercase tracking-tight">{service}</span>
                            <span className="text-base md:text-lg text-primary-500 drop-shadow-[0_0_15px_rgba(59,130,246,0.6)]">●</span>
                        </div>
                    ))}
